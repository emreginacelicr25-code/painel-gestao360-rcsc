...
import { BookOpen, Plus, X, ClipboardList, CheckCircle2, Calendar, Printer } from 'lucide-react'
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="tela-normal">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6 bg-white rounded-xl border border-night/10 p-4">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1.5 border border-night/15 text-night px-3 py-1.5 rounded-lg text-sm font-medium disabled:opacity-40"
          >
      ...
      <div className="area-impressao">
        <table style={{ width: '100%', borderBottom: '2px solid #1a2b4a', paddingBottom: 12, marginBottom: 16 }}>
          <tbody>
            <tr>
              <td style={{ width: 70, verticalAlign: 'middle' }}>
                <img src={brasao} alt="Brasão" style={{ width: 60, height: 60, objectFit: 'contain' }} />
              </td>
              <td style={{ verticalAlign: 'middle', textAlign: 'center' }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 'bold', color: '#1a2b4a' }}>
                  PREFEITURA MUNICIPAL DE DUQUE DE CAXIAS
                </p>
                <p style={{ margin: 0, fontSize: 12, color: '#1a2b4a' }}>SECRETARIA MUNICIPAL DE EDUCAÇÃO</p>
                <p style={{ margin: '2px 0 0', fontSize: 12, fontWeight: 'bold' }}>
                  E.M. REGINA CELI DA SILVA CERDEIRA
                </p>
                <p style={{ margin: 0, fontSize: 10, color: '#444' }}>Centro de Referência em Educação Inclusiva</p>
              </td>
              <td style={{ width: 70 }} />
            </tr>
          </tbody>
        ...
        <h2 style={{ textAlign: 'center', fontSize: 15, margin: '8px 0 2px', color: '#1a2b4a' }}>
          ATA DE REUNIÃO DE EQUIPE TÉCNICO-PEDAGÓGICA
        </h2>
        <p style={{ textAlign: 'center', fontSize: 12, margin: '0 0 20px', color: '#444' }}>
          Reunião de {formatarData(dataSelecionada)}
        </p>

        {pautaItens.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <h3 style={{ fontSize: 12.5, borderBottom: '1px solid #ccc', paddingBottom: 4, color: '#1a2b4a' }}>
              I — Pauta da Reunião
            </h3>
            {pautaItens.map((item) => (
              <p key={item.id} style={{ fontSize: 11.5, margin: '8px 0', lineHeight: 1.5 }}>
                • {item.conteudo}
                <br />
        ...
        {ataItens.length > 0 && (
          <div>
            ...
            {Object.entries(CATEGORIAS).map(([chave, { label }]) =>
              ataAgrupada[chave].length > 0 ? (
                <div key={chave} style={{ marginTop: 10 }}>
                  <p style={{ fontSize: 11.5, fontWeight: 'bold', margin: '6px 0 2px' }}>{label}</p>
                  {ataAgrupada[chave].map((item) => (
                    <p key={item.id} style={{ fontSize: 11.5, margin: '4px 0 4px 10px', lineHeight: 1.5 }}>
                      • {item.conteudo}
                      <br />
                      <span style={{ fontSize: 9.5, color: '#666' }}>
                        {item.autor_nome} — {formatarDataHora(item.criado_em)}
        ...
        <p style={{ fontSize: 11, marginTop: 36, textAlign: 'right' }}>
          Duque de Caxias, {formatarDataExtenso(dataSelecionada)}.
        </p>

        <div style={{ marginTop: 40 }}>
          {SIGNATARIOS.map((cargo) => (
            <div key={cargo} style={{ marginTop: 30, textAlign: 'center' }}>
              <div style={{ borderTop: '1px solid #1a1a1a', width: 280, margin: '0 auto' }} />
              <p style={{ fontSize: 11, margin: '4px 0 0' }}>{cargo}</p>
            </div>
          ))}
        </div>
      </div>
